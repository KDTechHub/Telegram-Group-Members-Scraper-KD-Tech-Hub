...
inf = (
    y + 'T' + a + 'E' + b + 'L' + y + 'E' + m + 'G' + c + 'R' + r + 'A' + y + 'M' + '  ' +
    y + 'S' + a + 'C' + b + 'R' + y + 'A' + m + 'P' + c + 'E' + r + 'R' + y + '  ' +
    '2' + y + '0' + a + '2' + b + '5' + y + '  ' + m + 'B' + c + 'Y' + r + '  ' +
    y + 'K' + 'U' + y + 'A' + a + 'D' + b + 'U' + y + 'G' + a + 'A' + c + 'H' + '  ' +
    y + 'D' + m + 'E' + c + 'L' + r + 'I' + y + 'G' + 'H' + 'T'
)
el=0
def Sleep(timE):
    try:
...
def ospath():
    ...
    for po in range(o):
        if os.path.isfile('multi_log.txt'):
            ...
            z=0
        api_id= input(b+' Enter your api_id_{}: '.format(z+1))
        api_hash= input('Enter your api_hash_{}: '.format(z+1))
        with open('multi_log.txt', 'a') as f:
            f.write(api_id+'\n'+api_hash+'\n')
        client = TelegramClient("KuadugahDelight{}".format(z), api_id, api_hash)
        client.start()
        Sleep(1)
        clear()
        info()
        client.disconnect()
if os.path.isfile('multi_log.txt'):
    xc=input(b+" Do you want to continue the last session "+a+" (y/n) ? ")
    if xc=='y':
        ...
        cv=input(" Do you want to remove the last session "+a+" (y/n) ? ")
        if cv=='y':
            with open('multi_log.txt', 'r') as f:
                data = f.readlines()
            v=int((len(data))/2)
            con=input(r+" Are you sure you want to permanently delete all files related to last session "+a+" (y/n) ? ")
            if con in ['', 'n']:
                ...
                Sleep(1)
                for d in range(v-1):
                    os.remove("KuadugahDelight{}.session".format(d))
...
for s in range(v):
    ...
    Sleep(1)
    client = TelegramClient("KuadugahDelight{}".format(x), api_id, api_hash)
    client.start()
    ...
    if g_index == '' :
        ...
        with open("ScrapedMembers.csv","w",encoding='UTF-8') as f:
    ...
    if g_index=='':
        print(m+" \n You 've pressed Enter,Now exiting...")
        sys.exit()  
    users = []  
    with open('ScrapedMembers.csv', encoding='UTF-8') as f:
    ...
    for user in users:
        ...
        if user['id'] in my_participants_id:
            ...
            n-=1
            with open('ScrapedMembers.csv', 'r',encoding='UTF-8') as f:
            ...
            try:
                ...
                print(r+' User in too many channel. Skipping.')
                with open('ScrapedMembers.csv', 'r',encoding='UTF-8') as f:
                    dat = csv.reader(f,delimiter=",",lineterminator="\n")
                    for tad in dat:
                        if usR in tad:
